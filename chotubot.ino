...
FirebaseData fbdo;       // used for all reads/writes (status updates + polling for commands)
FirebaseAuth fbAuth;
...
void setup() {
  ...
  // Boot sequence: a deliberate blank pause, then the branded intro, then
...
void handleIncomingMessage(const String &msg) {
  ...
  if (type == "expression") {
    ...
    pendingNotificationText = String((const char*)(doc["title"] | "")) + ": " +
                               String((const char*)(doc["body"] | ""));
    unsigned long dur = doc["durationMs"] | 6000;
    faces_glanceAt(0, -6, 600); // brief alert glance upward
